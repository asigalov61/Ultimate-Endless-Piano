...
Original file is located at
    https://colab.research.google.com/drive/1AJICJeB-8S9-DN49zUuMJnU_mmc3kApY
...
for j in notebook.tqdm_notebook(range(num_tries)):
  ...
  if counter == num_chords:
    ...
    for c in notebook.tqdm_notebook(melody_chords_f, disable=tqdmdis):
      ...
      for i in range(len(comp)-min(len(new_chords)-1, len(comp)-1)):

        chunk = comp[i:i+min(len(new_chords)-1, len(comp)-1)]

        match_ratio = sum([1 if a == b else 0 for a, b in zip(new_chords[:len(chunk)], chunk[:len(new_chords)])]) / min(len(new_chords)-1, len(comp)-1)
        match_ratios.append(match_ratio)

        if match_ratio == 1.0:
          break

      if max(match_ratios) == 1.0:
        if verbose:
      ...
      elif max_overfit_ratio < max(match_ratios) < 1.0:
        if verbose:
          print('High overfit:', max(match_ratios))
          print('Starting new search...')
          print('=' * 70)
        break
